...
import { assignChallenge, getChallenges, getCurrentUserFromStorage, getUniversities, updateChallengePriority, updateChallengeStatus } from '@/lib/api'
...
function SectionTitle({ eyebrow, title, action }: { eyebrow: string; title: string; action?: string }) {
  return <div className="mb-5 flex items-end justify-between gap-4"><div><p className="text-[11px] font-bold uppercase tracking-wider text-orange-700">{eyebrow}</p><h2 className="mt-1 text-lg font-bold text-slate-950">{title}</h2></div>{action && <button className="hidden items-center gap-1 text-xs font-bold text-emerald-800 sm:flex">{action}<ArrowUpRight className="size-3.5" /></button>}</div>
}
...
export default function GovernmentDashboard() {
  ...
  const action = (label: string) => { if (label.startsWith('View ')) { setSelectedChallenge(challenges.find((challenge) => challenge._id === label.slice(5)) || null); return } setToast(`${label} selected`); window.setTimeout(() => setToast(''), 1800) }
  useEffect(() => {
  ...
  if (!challenges.length) return <div className="min-h-full bg-slate-50 p-5 sm:p-8"><div className="mx-auto max-w-[1450px] rounded-2xl border border-dashed border-slate-300 bg-white p-12 text-center"><p className="font-bold text-slate-800">No challenges available</p><p className="mt-1 text-sm text-slate-500">Government analytics will appear when challenges are submitted.</p></div></div>
  return <div className="relative min-h-full bg-slate-50 p-5 sm:p-8">
    ...
    <div className="mx-auto max-w-[1450px]">
      ...
      <div className="mt-6 grid gap-6 xl:grid-cols-[.85fr_1.15fr]">
        ...
        <section className="rounded-2xl border border-slate-200 bg-white p-6"><SectionTitle eyebrow="Triage queue" title="Challenges requiring attention" action="Review queue" /><div className="overflow-x-auto"><table className="w-full min-w-[760px] text-left text-xs"><thead><tr className="border-b border-slate-100 text-[10px] uppercase tracking-wider text-slate-400"><th className="pb-3 pr-4">Challenge</th><th className="pb-3 pr-4">District</th><th className="pb-3 pr-4">Priority</th><th className="pb-3 pr-4">Status</th><th className="pb-3 pr-4">Submitted</th><th className="pb-3">Action</th></tr></thead><tbody>{attentionRows.map(row => <tr key={row[0]} className="border-b border-slate-50 last:border-0"><td className="py-3 pr-4"><p className="font-bold text-emerald-800">{row[0]}</p><p className="mt-1 max-w-52 font-semibold leading-4 text-slate-700">{row[1]}</p><p className="mt-1 text-slate-400">{row[3]}</p></td><td className="py-3 pr-4 font-medium text-slate-600">{row[2]}</td><td className="py-3 pr-4"><select disabled={!canMutate} value={row[8]} onChange={(event) => changePriority(row[0], event.target.value)} className="rounded-full border-0 bg-slate-100 px-2 py-1 text-xs font-bold text-slate-700"><option value="low">Low</option><option value="medium">Medium</option><option value="high">High</option><option value="critical">Critical</option></select></td><td className="py-3 pr-4"><select disabled={!canMutate} value={row[7]} onChange={(event) => changeStatus(row[0], event.target.value)} className="rounded-full border-0 bg-slate-100 px-2 py-1 text-xs font-semibold text-slate-600"><option value="submitted">Submitted</option><option value="under_review">Under review</option><option value="approved">Approved</option><option value="assigned">Assigned</option><option value="in_progress">In progress</option><option value="resolved">Resolved</option><option value="rejected">Rejected</option></select></td><td className="py-3 pr-4 whitespace-nowrap text-slate-500">{row[6]}</td><td className="py-3"><button onClick={() => action(`View ${row[0]}`)} className="font-bold text-emerald-800 hover:underline">View</button><button disabled={!canMutate} title={canMutate ? 'Select a university to assign this challenge' : 'Government access required'} onClick={() => openAssignment(row[0], row[1])} className="ml-3 font-bold text-emerald-800 disabled:cursor-not-allowed disabled:text-slate-400">Assign</button></td></tr>)}</tbody></table></div></section>
      </div>

      <div className="mt-6 grid gap-6 xl:grid-cols-[1.15fr_.85fr]">
        <section className="rounded-2xl border border-slate-200 bg-white p-6"><SectionTitle eyebrow="Institutional network" title="University participation" action="View universities" /><div className="overflow-x-auto"><table className="w-full min-w-[560px] text-left text-xs"><thead><tr className="border-b border-slate-100 text-[10px] uppercase tracking-wider text-slate-400"><th className="pb-3">University name</th><th className="pb-3">Assigned</th><th className="pb-3">Active</th><th className="pb-3">Completed</th><th className="pb-3">Status</th></tr></thead><tbody>{universities.map((university) => <tr key={university._id} className="border-b border-slate-50 last:border-0"><td className="py-3 font-bold text-slate-700">{university.institution || university.name}</td><td className="py-3 font-semibold text-slate-600">{challenges.filter((challenge) => challenge.assignedUniversity?._id === university._id).length}</td><td className="py-3 font-semibold text-slate-600">{challenges.filter((challenge) => challenge.assignedUniversity?._id === university._id && challenge.status !== 'resolved').length}</td><td className="py-3 font-semibold text-slate-600">{challenges.filter((challenge) => challenge.assignedUniversity?._id === university._id && challenge.status === 'resolved').length}</td><td className="py-3"><span className="rounded-full bg-emerald-50 px-2 py-1 font-bold text-emerald-700">Registered</span></td></tr>)}</tbody></table></div></section>
