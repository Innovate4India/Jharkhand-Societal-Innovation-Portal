...
import { ArrowUpRight, BriefcaseBusiness, CheckCircle2, ClipboardList, Plus, Users, UserRound, Wrench, X } from 'lucide-react'
import { acceptChallenge, getChallenges, getCollaborations, getCurrentUser, getCurrentUserFromStorage, getProjects, getUniversityMembers, updateCollaborationStatus, updateProjectStatus, updateProjectTeam } from '@/lib/api'
import { createProject } from '@/lib/api'

const stages = ['Assigned Problem', 'Department', 'Faculty', 'Student Team', 'Work on Solution', 'Testing', 'Completed']

function Stat({ label, value, icon: Icon, note }: { label: string; value: string; icon: typeof Users; note: string }) { return <div className="rounded-2xl border border-slate-200 bg-white p-5"><div className="flex items-start justify-between"><div><p className="text-sm text-slate-500">{label}</p><p className="mt-2 text-3xl font-bold text-slate-950">{value}</p></div><span className="grid size-10 place-items-center rounded-xl bg-emerald-50 text-emerald-700"><Icon className="size-5" /></span></div><p className="mt-4 text-xs font-semibold text-emerald-700">{note}</p></div> }
...
function ChallengeDetailsModal({ challenge, onClose }: { challenge: Awaited<ReturnType<typeof getChallenges>>['data'][number]; onClose: () => void }) {
  ...
  return <div className="fixed inset-0 z-50 grid place-items-center bg-slate-950/40 p-4"><div role="dialog" aria-modal="true" aria-labelledby="university-challenge-details-title" className="max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-2xl bg-white p-6 shadow-2xl"><div className="flex items-start justify-between gap-4"><div><p className="text-[11px] font-bold uppercase tracking-wider text-orange-700">Challenge details</p><h2 id="university-challenge-details-title" className="mt-1 text-xl font-bold text-slate-950">{challenge.title || 'Not available'}</h2></div><button type="button" onClick={onClose} aria-label="Close challenge details"><X className="size-5 text-slate-500" /></button></div><div className="mt-6 grid gap-4 sm:grid-cols-2">{fields.map(([label, value]) => <div key={label} className="rounded-lg bg-slate-50 p-3"><p className="text-[11px] font-bold uppercase tracking-wider text-slate-400">{label}</p><p className="mt-1 whitespace-pre-wrap text-sm font-semibold text-slate-800">{value === undefined || value === null || value === '' ? 'Not available' : String(value)}</p></div>)}</div><button type="button" onClick={onClose} className="mt-6 w-full rounded-lg bg-emerald-800 px-4 py-3 text-sm font-bold text-white">Close</button></div></div>
}
...
export default function UniversityDashboard() {
  ...
  return <div className="relative min-h-full bg-slate-50 p-5 sm:p-8">{showTeamEditor && <TeamEditor members={universityMembers || []} selection={teamSelection} saving={savingTeam} onToggle={(id, checked) => setTeamSelection(checked ? [...teamSelection, id] : teamSelection.filter((selectedId) => selectedId !== id))} onSave={() => void saveTeam()} onClose={() => setShowTeamEditor(false)} />}{showProfile && currentUser && <ProfileModal user={currentUser} onClose={() => setShowProfile(false)} />}{toast && <div className="fixed bottom-5 right-5 z-50 rounded-xl bg-emerald-900 px-4 py-3 text-sm font-semibold text-white shadow-lg">{toast}</div>}{selectedChallenge && <ChallengeDetailsModal challenge={selectedChallenge} onClose={() => setSelectedChallenge(null)} />}<div className="mx-auto max-w-[1450px]">{(!relevantChallenges.length || !relevantProjects.length) && <div className="mb-6 rounded-xl border border-dashed border-slate-300 bg-white p-4 text-sm text-slate-500">{!relevantChallenges.length && !relevantProjects.length ? 'No assigned challenges or projects are available for this university yet.' : !relevantChallenges.length ? 'No assigned challenges are available for this university yet.' : 'No projects are available for this university yet.'}</div>}
    <div className="flex flex-col justify-between gap-4 md:flex-row md:items-end"><div><p className="text-xs font-medium text-slate-400">Higher education innovation workspace</p><h2 className="mt-1 text-2xl font-bold tracking-tight text-slate-950 sm:text-3xl">University Dashboard</h2><p className="mt-2 max-w-2xl text-sm leading-6 text-slate-500">Discover challenges, build multidisciplinary teams and develop solutions for communities.</p></div><button onClick={() => setShowProfile(true)} className="flex w-fit items-center gap-2 rounded-lg border border-slate-200 bg-white px-4 py-2.5 text-sm font-bold text-slate-700">My Profile</button></div>
    <div className="mt-7 grid gap-4 sm:grid-cols-2 xl:grid-cols-6"><Stat label="Assigned Challenges" value={String(assignedCount)} note="From current API data" icon={ClipboardList} /><Stat label="Challenges Accepted" value={String(acceptedCount)} note="Current API data" icon={CheckCircle2} /><Stat label="Active Projects" value={String(activeCount)} note="From current API data" icon={Wrench} /><Stat label="Completed Projects" value={String(completedCount)} note="From current API data" icon={BriefcaseBusiness} /><Stat label="Students Involved" value={String(studentCount)} note="From project team data" icon={Users} /><Stat label="Faculty Mentors" value={String(facultyCount)} note="From university members" icon={UserRound} /></div>
