...
export default function UniversityDashboard() {
  ...
  const projectReadyChallenges = availableChallenges.filter((challenge) => challenge.assignmentStatus === 'accepted' && challenge.fundingStatus === 'approved')
  ...
  return <div className="mobile-role-dashboard relative min-h-full min-w-0 bg-slate-50 p-5 sm:p-8">{showTeamEditor && <TeamEditor members={universityMembers || []} selection={teamSelection} saving={savingTeam} onToggle={(id, checked) => setTeamSelection(checked ? [...teamSelection, id] : teamSelection.filter((selectedId) => selectedId !== id))} onSave={() => void saveTeam()} onClose={() => setShowTeamEditor(false)} />}{showProfile && currentUser && <ProfileModal user={currentUser} onClose={() => setShowProfile(false)} />}{toast && <div className="fixed bottom-5 right-5 z-50 rounded-xl bg-emerald-900 px-4 py-3 text-sm font-semibold text-white shadow-lg">{toast}</div>}{selectedChallenge && <ChallengeDetailsModal challenge={selectedChallenge} onClose={() => setSelectedChallenge(null)} />}<div className="mx-auto max-w-[1450px]">{(!relevantChallenges.length || !relevantProjects.length) && <div className="mb-6 rounded-xl border border-dashed border-slate-300 bg-white p-4 text-sm text-slate-500">{!relevantChallenges.length && !relevantProjects.length ? 'No assigned challenges or projects are available for this university yet.' : !relevantChallenges.length ? 'No assigned challenges are available for this university yet.' : 'No projects are available for this university yet.'}</div>}
    ...
    <div className="mt-8"><Section eyebrow="AI-powered recommendations" title="Assigned Challenges"><div className="grid gap-4 lg:grid-cols-3">{challenges.map(c => <article key={c[0]} className="rounded-xl border border-slate-200 p-5"><div className="flex items-center justify-between"><span className="text-xs font-bold text-emerald-800">{c[0]}</span><div className="flex items-center gap-2"><Pill tone={c[4] === 'High' ? 'red' : 'orange'}>{c[4]} priority</Pill><Pill tone={c[8] === 'accepted' ? 'green' : 'slate'}>{c[8] === 'accepted' ? 'Accepted' : 'Awaiting your acceptance'}</Pill></div></div><h3 className="mt-4 text-base font-bold leading-6 text-slate-950">{c[1]}</h3><p className="mt-1 text-xs font-medium text-slate-500">{c[3]} · {c[2]}</p><p className="mt-3 text-sm leading-6 text-slate-600">{c[6]}</p><p className="mt-3 text-xs font-bold text-orange-700">{c[8] === 'accepted' && c[9] !== 'approved' ? 'Waiting for Government Funding' : c[9] === 'approved' ? 'Funding approved - ready for project creation' : ''}</p><div className="mt-4 flex items-center justify-between border-t border-slate-100 pt-4"><span className="text-xs font-bold text-emerald-700">AI Match: {c[5]}</span><div className="flex gap-2"><button onClick={() => action(`View ${c[0]}`)} className="rounded-lg border border-slate-200 px-3 py-2 text-xs font-bold text-slate-600">View Challenge</button><button disabled={c[8] === 'accepted' || acceptingChallengeId === c[0]} onClick={() => action(`Accept ${c[0]}`)} className="rounded-lg bg-emerald-800 px-3 py-2 text-xs font-bold text-white disabled:cursor-not-allowed disabled:opacity-50">{c[8] === 'accepted' ? 'Accepted' : acceptingChallengeId === c[0] ? 'Accepting...' : 'Accept'}</button></div></div></article>)}</div></Section></div>
