...
import { acceptChallenge, acceptUniversitySponsorship, downloadChallengeAttachment, getChallenges, getCollaborations, getCurrentUser, getCurrentUserFromStorage, getProjects, getUniversityMembers, getUniversitySponsorships, updateCollaborationStatus, updateProjectStatus, updateProjectTeam } from '@/lib/api'
...
export default function UniversityDashboard() {
  ...
  const challenges = relevantChallenges.map((challenge) => [challenge._id, challenge.title, challenge.district, challenge.category, challenge.priority === 'high' || challenge.priority === 'critical' ? 'High' : challenge.priority === 'low' ? 'Low' : 'Medium', '—', challenge.description, challenge.status, challenge.assignmentStatus, challenge.fundingStatus])
  ...
  async function handleAcceptSponsorship(challengeId: string) {
    ...
    if (!response.success) showToast(response.message || 'Unable to accept Industry funding')
      ...
      showToast('Industry funding accepted')
    }
  ...
  return <div className="mobile-role-dashboard relative min-h-full min-w-0 bg-slate-50 p-5 sm:p-8">{showTeamEditor && <TeamEditor members={universityMembers || []} selection={teamSelection} saving={savingTeam} onToggle={(id, checked) => setTeamSelection(checked ? [...teamSelection, id] : teamSelection.filter((selectedId) => selectedId !== id))} onSave={() => void saveTeam()} onClose={() => setShowTeamEditor(false)} />}{showProfile && currentUser && <ProfileModal user={currentUser} onClose={() => setShowProfile(false)} />}{toast && <div className="fixed bottom-5 right-5 z-50 rounded-xl bg-emerald-900 px-4 py-3 text-sm font-semibold text-white shadow-lg">{toast}</div>}{selectedChallenge && <ChallengeDetailsModal challenge={selectedChallenge} onClose={() => setSelectedChallenge(null)} />}<div className="mx-auto max-w-[1450px]">{(!relevantChallenges.length || !relevantProjects.length) && <div className="mb-6 rounded-xl border border-dashed border-slate-300 bg-white p-4 text-sm text-slate-500">{!relevantChallenges.length && !relevantProjects.length ? 'No assigned challenges or projects are available for this university yet.' : !relevantChallenges.length ? 'No assigned challenges are available for this university yet.' : 'No projects are available for this university yet.'}</div>}
    ...
    <div className="mt-8"><Section eyebrow="AI-powered recommendations" title="Assigned Challenges"><div className="grid gap-4 lg:grid-cols-3">{challenges.map(c => <article key={c[0]} className="rounded-xl border border-slate-200 p-5"><div className="flex items-center justify-between"><span className="text-xs font-bold text-emerald-800">{c[0]}</span><div className="flex items-center gap-2"><Pill tone={c[7] === 'cancelled' ? 'red' : c[8] === 'accepted' ? 'green' : 'slate'}>{c[7] === 'cancelled' ? 'Cancelled' : c[8] === 'accepted' ? 'Accepted' : 'Awaiting your acceptance'}</Pill></div></div><h3 className="mt-4 text-base font-bold leading-6 text-slate-950">{c[1]}</h3><p className="mt-1 text-xs font-medium text-slate-500">{c[3]} · {c[2]}</p><p className="mt-3 text-sm leading-6 text-slate-600">{c[6]}</p><p className="mt-3 text-xs font-bold text-orange-700">{c[7] === 'cancelled' ? 'Cancelled' : c[8] !== 'accepted' ? 'Waiting for University Acceptance' : 'Open for Industry Funding'}</p><div className="mt-4 flex items-center justify-between border-t border-slate-100 pt-4"><span className="text-xs font-bold text-emerald-700">AI Match: {c[5]}</span><div className="flex gap-2"><button onClick={() => action(`View ${c[0]}`)} className="rounded-lg border border-slate-200 px-3 py-2 text-xs font-bold text-slate-600">View Challenge</button><button disabled={c[7] === 'cancelled' || c[8] === 'accepted' || acceptingChallengeId === c[0]} onClick={() => action(`Accept ${c[0]}`)} className="rounded-lg bg-emerald-800 px-3 py-2 text-xs font-bold text-white disabled:cursor-not-allowed disabled:opacity-50">{c[7] === 'cancelled' ? 'Cancelled' : c[8] === 'accepted' ? 'Accepted' : acceptingChallengeId === c[0] ? 'Accepting...' : 'Accept'}</button></div></div></article>)}</div></Section></div>
    <div className="mt-8"><Section eyebrow="Industry funding received" title="Funding requiring your acceptance"><div className="grid gap-4 lg:grid-cols-2">{sponsorshipData.length ? sponsorshipData.map((sponsorship) => <article key={sponsorship._id} className="rounded-xl border border-slate-200 p-5"><p className="text-sm font-bold text-slate-900">{sponsorship.challenge?.title || 'Assigned problem'}</p><p className="mt-2 text-sm text-slate-600">Company: <b>{sponsorship.industry?.organizationName || sponsorship.industry?.name || 'Industry partner'}</b></p><p className="mt-1 text-sm text-slate-600">Amount: <b>₹{Number(sponsorship.amount || 0).toLocaleString('en-IN')}</b></p><p className="mt-1 text-sm text-slate-600">Status: <b>{sponsorship.status === 'accepted' || sponsorship.challenge?.industryFundingStatus === 'accepted' ? 'Accepted' : 'Pending University Acceptance'}</b></p>{sponsorship.status === 'pending' && <button onClick={() => void handleAcceptSponsorship(sponsorship.challenge?._id)} disabled={acceptingSponsorshipId === sponsorship.challenge?._id} className="mt-4 rounded-lg bg-emerald-800 px-4 py-2 text-xs font-bold text-white disabled:opacity-50">{acceptingSponsorshipId === sponsorship.challenge?._id ? 'Accepting...' : 'Accept Funding'}</button>}</article>) : <p className="rounded-xl border border-dashed border-slate-300 bg-slate-50 p-5 text-sm text-slate-500">No Industry funding has been received yet.</p>}</div></Section></div>
