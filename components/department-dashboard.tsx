...
export default function DepartmentDashboard({ user, section = 'problems', onSectionChange }: { user: User; section?: DepartmentSection; onSectionChange?: (section: DepartmentSection) => void }) {
  ...
  return <main className="min-h-full bg-slate-50 p-5 sm:p-8"><div className="mx-auto max-w-[1350px]">
    ...
    <div id="department-progress" className="mt-6"><Card eyebrow="Persistent project tracking" title="Progress"><div className="space-y-3">{projects.map((project) => { const currentStage = stageLabels.includes(project.currentStage || '') ? project.currentStage || 'proposed' : 'proposed'; const currentIndex = stageLabels.indexOf(currentStage); return <div key={project._id} className="rounded-xl border border-slate-200 p-4"><p className="font-bold text-slate-900">{project.title}</p><p className="mt-2 text-sm font-semibold text-slate-700">Current Stage: {currentStage}</p><div className="mt-4 space-y-2">{stageLabels.map((stage, index) => { const stageUpdates = project.progressUpdates?.filter((item) => item.stage === stage) || []; const historicalUpdate = stageUpdates.reduce((latest, update) => !latest || new Date(update.updatedAt || 0).getTime() >= new Date(latest.updatedAt || 0).getTime() ? update : latest, undefined); return <div key={stage} className="flex gap-2 text-sm"><span className="w-4 text-slate-500">{index <= currentIndex ? '✓' : '○'}</span><div><p className="font-semibold capitalize text-slate-700">{stage}</p>{historicalUpdate && <p className="text-xs text-slate-500">{historicalUpdate.description}</p>}</div></div> })}</div></div> })}{!projects.length && <p className="text-sm text-slate-500">Progress appears after a funded project is created.</p>}</div></Card></div>
